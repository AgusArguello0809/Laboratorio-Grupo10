...
const Carrito = () => {
  ...
  const disminuirCantidad = async (id) => {
    ...
    try {
      await fetch(
        `${API_BASE_URL}/carritos/${carrito.id}/producto/${id}/disminuir`,
        {
          method: "PATCH",
          headers,
        }
  ...
  const aumentarCantidad = async (id) => {
    try {
      await fetch(
        `${API_BASE_URL}/carritos/${carritoId}/producto/${id}/incrementar`,
        {
          method: "PATCH",
          headers,
        }
  ...
  const confirmarCantidad = async (id) => {
    ...
    try {
      await fetch(
        `${API_BASE_URL}/carritos/${carritoId}/producto/${id}/cantidad`,
        {
          method: "PATCH",
          headers,
  ...
  const handleCheckout = async () => {
    try {
      const res = await fetch(
        `${API_BASE_URL}/carritos/${carritoId}/checkout`,
        {
          method: "POST",
          headers,
        }
      ...
      setSnackbar({
        open: true,
        message: e.message || "Error al hacer checkout",
        severity: "error",
      });
    }
  };
              ...
              <CardContent>
                ...
                <Typography variant="subtitle2" sx={{ mb: 1 }}>
                  ...
                  {(
                    (isNaN(parseFloat(item.price))
                      ? 0
                      : parseFloat(item.price)) * Number(item.cantidad || 0)
                  ).toFixed(2)}
                </Typography>
                <CardActions sx={{ gap: 1 }}>
                  ...
                  <TextField
                    ...
                    onChange={(e) =>
                      manejarInputTemp(item.productoId, e.target.value)
                    }
                    sx={{ width: 90 }}
                    inputProps={{
